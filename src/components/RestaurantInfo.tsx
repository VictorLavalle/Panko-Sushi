  ...
  return (
    <section id="contact" className="px-4 py-10 scroll-mt-24" aria-label={t("contact.section")}>
      ...
      <div className="bg-[var(--color-bg-card)] rounded-2xl p-5 border border-[var(--color-border)] grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        ...
        <div className="flex items-center gap-3">
          <svg className="w-5 h-5 text-[var(--color-text-secondary)]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--color-text-secondary)] font-medium hover:text-[var(--color-text)] transition-colors">
            {instagram}
          </a>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg" aria-hidden="true">🕐</span>
          <span className="text-[var(--color-text-secondary)]">{hours}</span>
        </div>
      </div>
    </section>
  );
